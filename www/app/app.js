(function() {
  'use strict';
  angular.module('app', ['ionic', 'lbServices', 'app.signin', 'app.signup', 'ionic-toast', 'app.social'])
    .config(configBlock)
    .run(runBlock);
  ...
  function configBlock($stateProvider, $urlRouterProvider, $provide) {
    ...
    $provide.decorator('$exceptionHandler', ['$delegate', function($delegate) {
      return function(exception, cause) {
        ...
        Logger.error('Angular error: ' + data.message, {
          cause: data.cause,
          stack: data.stack
        });
      };
  ...
  window.onerror = function(message, url, line, col, error) {
    ...
    Logger.error('JavaScript error: ' + data.message, {
      cause: data.cause,
      stack: data.stack
    });
